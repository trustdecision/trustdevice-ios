...
#import "TongdunMobRiskSafeDictionary.h"
...
/// SDK Object
static TongdunMobRiskManager_t *riskManager;
/// SDKVersion
static NSString *sdkVersion = @"1.0";
/// CollectData Dictionary
static TongdunMobRiskSafeDictionary *infoDict;
...
/// Assert
static void tongdunmobrisk_assert(BOOL condition, const char *assertDescription);

@implementation TongdunMobRiskManager
...
+ (TongdunMobRiskManager_t *)sharedManager {
    ...
    dispatch_once(&deviceManager_once_token, ^{
        riskManager = malloc(sizeof(TongdunMobRiskManager_t));
...
void initWithOptions(NSDictionary *options) {
    ...
    tongdunmobrisk_assert([options isKindOfClass:[NSDictionary class]], ("[TongdunMobRiskManager] - [INIT_ERROR] - options is not dictonary type"));
    // Async Collect
    dispatch_barrier_async(collectQueue, ^{
        // Response data
        NSMutableDictionary *dataDict = [[NSMutableDictionary alloc] init];
        // Collect data
        NSDictionary *collectInfo = [TongdunMobRiskCollector getCollectInfo];
        // Calculate and set deviceId
        NSString *deviceId = [TongdunMobRiskIdCalculator generateIdByInfo:collectInfo];
        if (deviceId) {
            [dataDict setObject:deviceId forKey:DeviceIdKey];
        }
        // Calculate and set deviceRisk
        NSDictionary *deviceRiskLabel = [TongdunMobRiskRiskCalculator generateRiskLabelByInfo:collectInfo];
        if (deviceRiskLabel) {
        ...
        infoDict = [[TongdunMobRiskSafeDictionary alloc] initWithDictionary:dataDict];
